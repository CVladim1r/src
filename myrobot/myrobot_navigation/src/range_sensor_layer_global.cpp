#include <range_sensor_layer_global.h>
...
PLUGINLIB_EXPORT_CLASS(range_sensor_layer_global::RangeSensorLayerGlobal, costmap_2d::Layer)

using costmap_2d::NO_INFORMATION;

namespace range_sensor_layer_global
{

RangeSensorLayerGlobal::RangeSensorLayerGlobal() {}

void RangeSensorLayerGlobal::onInitialize()
{
  ...
  for (int i = 0; i < topic_names.size(); i++)
  {
    ...
    else
    {
      ...
      if (input_sensor_type == VARIABLE)
        processRangeMessageFunc_ = boost::bind(&RangeSensorLayerGlobal::processVariableRangeMsg, this, _1);
      else if (input_sensor_type == FIXED)
        processRangeMessageFunc_ = boost::bind(&RangeSensorLayerGlobal::processFixedRangeMsg, this, _1);
      else if (input_sensor_type == ALL)
        processRangeMessageFunc_ = boost::bind(&RangeSensorLayerGlobal::processRangeMsg, this, _1);
      else
      {
        ROS_ERROR(
      ...
      range_subs_.push_back(nh.subscribe(topic_name, 100, &RangeSensorLayerGlobal::bufferIncomingRangeMsg, this));

      ROS_INFO("RangeSensorLayerGlobal: subscribed to topic %s", range_subs_.back().getTopic().c_str());
    }
  }

  dsrv_ = new dynamic_reconfigure::Server<range_sensor_layer_global::RangeSensorLayerGlobalConfig>(nh);
  dynamic_reconfigure::Server<range_sensor_layer_global::RangeSensorLayerGlobalConfig>::CallbackType cb =
    boost::bind(&RangeSensorLayerGlobal::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
  global_frame_ = layered_costmap_->getGlobalFrameID();
}


double RangeSensorLayerGlobal::gamma(double theta)
{
...
double RangeSensorLayerGlobal::delta(double phi)
{
  return 1 - (1 + tanh(2 * (phi - phi_v_))) / 2;
}

void RangeSensorLayerGlobal::get_deltas(double angle, double *dx, double *dy)
{
  double ta = tan(angle);
  if (ta == 0)
    *dx = 0;
...
double RangeSensorLayerGlobal::sensor_model(double r, double phi, double theta)
{
...
void RangeSensorLayerGlobal::reconfigureCB(range_sensor_layer_global::RangeSensorLayerGlobalConfig &config, uint32_t level)
{
  phi_v_ = config.phi;
...
void RangeSensorLayerGlobal::bufferIncomingRangeMsg(const sensor_msgs::RangeConstPtr& range_message)
{
  boost::mutex::scoped_lock lock(range_message_mutex_);
  range_msgs_buffer_.push_back(*range_message);
}

void RangeSensorLayerGlobal::updateCostmap()
{
...
void RangeSensorLayerGlobal::processRangeMsg(sensor_msgs::Range& range_message)
{
...
void RangeSensorLayerGlobal::processFixedRangeMsg(sensor_msgs::Range& range_message)
{
...
void RangeSensorLayerGlobal::processVariableRangeMsg(sensor_msgs::Range& range_message)
{
...
void RangeSensorLayerGlobal::updateCostmap(sensor_msgs::Range& range_message, bool clear_sensor_cone)
{
...
void RangeSensorLayerGlobal::update_cell(double ox, double oy, double ot, double r, double nx, double ny, bool clear)
{
  unsigned int x, y;
...
void RangeSensorLayerGlobal::updateBounds(double robot_x, double robot_y, double robot_yaw,
...
void RangeSensorLayerGlobal::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void RangeSensorLayerGlobal::reset()
{
  ROS_DEBUG("Reseting range sensor layer global...");
  deactivate();
  resetMaps();
  current_ = true;
  activate();
...
void RangeSensorLayerGlobal::deactivate()
{
  range_msgs_buffer_.clear();
}

void RangeSensorLayerGlobal::activate()
{
  range_msgs_buffer_.clear();
}
